...
# Own modules
try:
    from pc_skeletor.pc_skeletor.download import *
    from pc_skeletor.pc_skeletor.base import *
except:
...
class LaplacianBasedContractionBase(SkeletonBase):
    ...
    def __init__(self,
                 algo_type: str,
                 point_cloud: Union[str, np.ndarray, open3d.geometry.PointCloud, dict],
        ...
        if isinstance(point_cloud, str):
            ...
            self.pcd: o3d.geometry.PointCloud = points2pcd(point_cloud)
            ...
            elif isinstance(point_cloud['trunk'], np.ndarray):
                self.trunk: o3d.geometry.PointCloud = points2pcd(point_cloud['trunk'])
            else:
            ...
            elif isinstance(point_cloud['branches'], np.ndarray):
                self.branches: o3d.geometry.PointCloud = points2pcd(point_cloud['branches'])
            else:
        ...
        self.farthest_point_sampling = farthest_point_sampling
    ...
    def extract_skeleton(self):
        ...
        logging.info('Init value contraction: {}'.format(self.param_init_contraction))
        logging.info('Init value attraction: {}'.format(self.param_init_attraction))
        logging.info('Max contraction value: {}'.format(self.param_max_contraction))
        logging.info('Max attraction value: {}'.format(self.param_max_attraction))
        logging.info('Termination ratio: {}'.format(self.param_termination_ratio))
        logging.info('Amplification factor: {}'.format(self.param_contraction_amplification))

        L, M = robust_laplacian.point_cloud_laplacian(pcd_points, mollify_factor=1e-5, n_neighbors=30) # L = NxN, M = NxN
        # L = Laplacian matrix, M = Mass matrix
        M_list = [M.diagonal()]
        ...
        while np.mean(M_list[-1]) / np.mean(M_list[0]) > self.param_termination_ratio:
            ...
            logging.info('Laplacian Weight: {}'.format(laplacian_weights))
            logging.info('Mean Positional Weight: {}'.format(np.mean(positional_weights)))
    ...
    def extract_topology(self):
        logging.info('Extracting topology...')
        ...
        self.fps_points = int(contracted_point_cloud.shape[0] * self.farthest_point_sampling)
        ...
        logging.info('Topology extraction is done')
    ...
    def export_results(self, output: str):
        ...
        path_contracted_pcd = os.path.join(output, '00_input_{}'.format(self.algo_type) + '.ply')
        o3d.io.write_point_cloud(path_contracted_pcd, self.pcd)
...
class LBC(LaplacianBasedContractionBase):
    ...
    def __init__(self,
                 ...
                 farthest_point_sampling: float = 0.1,
                 debug: bool = False,
                 verbose: bool = False):
