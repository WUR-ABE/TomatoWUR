...
try:
	from pc_skeletor.pc_skeletor import SLBC, LBC
	from pc_skeletor.pc_skeletor import Dataset
except:
...
if __name__ == "__main__":
	...
	dt_graph_dir = Path("Resources/output") / config["skeleton_method"]

	plant_nr = "Harvest_03_PotNr_407"
	file_name = folder / "annotations"/ plant_nr / (plant_nr + ".csv")

	df = pd.read_csv(str(file_name), low_memory=False)
	df[['x', 'y', 'z']] = df[['x', 'y', 'z']]*1
	...
	pcd_trunk.points = o3d.utility.Vector3dVector(df.loc[df["semantic"]==2, ["x", "y", "z"]].values)
	pcd_branch = o3d.geometry.PointCloud()
	pcd_branch.points = o3d.utility.Vector3dVector(df.loc[df["semantic"]==4, ["x", "y", "z"]].values)
	# downloader = Dataset()
	...
	lbc = LBC(point_cloud=pcd,
			  ...
			  init_attraction=0.5,
			  down_sample=-1,
			  filter_nb_neighbors = None,
			  filter_std_ratio = None,
			  debug = False,
			  ) # was 0.03
